"""Bug Fixes
- Fixed glitch where selected car would default back to the randomly generated
option at the start of the game when entering the customise() function.
- Added more text to the instruction() function.
- Still draws AI cars in the Game Over screen.
- Adjusted spawn chance.
- Incrementally decreases the spawn cooldown as the sore gets higher.
- Fixed misalignment of user car selection display in customise().
"""
...
def instructions():
    ...
    while not back:  # returns once user has pressed the back button
        ...
        MISC_TEXT1 = DEFAULT_FONT.render("Use the customise button on the main", True, WHITE)
        MISC_TEXT2 = DEFAULT_FONT.render("menu to change your car colour.", True, WHITE)
...
def customise():
    ...
    # Get the index of the current user_car
    car_num = car_list.index([key for key, value in CARS.items() if value == user_car][0])
    ...
    while not back:  # returns once user has pressed the back button
        ...
        for display_car in car_list:
            ...
            coordinate = DISPLAY_COORDS[display_car]
        ...
        # Display the arrow sprite below the currently selected car
        arrow_rect = select_arrow.get_rect(center=(
            DISPLAY_COORDS[car_list[car_num]], y_value + 75))
...
def game_over_screen(user_score):
    ...
    while True:
        ...
        if user_score > high_score:
            new_highscore = GAME_OVER_SCORE_FONT.render(f"NEW HIGHSCORE!",
                                                        True, BLACK)
...
while running:
    ...
    if scroll_time >= 250:
        scroll_value += 0.20
        # Whenever the background scroll speed increases, also increase the
        # random chance for an AI car to spawn
