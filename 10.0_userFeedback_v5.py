"""User Feedback Component - Version 5
A component where I get end-user feedback and then trial their suggestions to
incorporate into my code.
- Changed the game_over_screen() function to not fill the screen with grey, so
that the text and buttons are generated over the actual screen -- gives a
smoother, less jarring appearance.
- Changed the "Play Again" button to a "Main Menu" button which return the user
to the main menu rather than immediately playing again.
"""
    ...
    for car in cars:
        ...
        if user_ai_collision(user_car, LANES[current_lane], cars):
            ...
            if game_over_screen(score):
                ...
                # Set new highscore if game continues
                if score > high_score:
                    high_score = score
